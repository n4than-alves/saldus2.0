...
serve(async (req) => {
  ...
  try {
    ...
    if (!user?.email || !user.id) throw new Error("Usuário não autenticado ou dados incompletos");
    logStep("Usuário autenticado", { userId: user.id, email: user.email });

    // Buscar stripe_customer_id salvo no perfil do usuário
    ...
    if (profileError || !profile?.stripe_customer_id) {
      throw new Error("stripe_customer_id ausente no perfil do usuário");
    }
